...
This page contains 10 DuckDB-style SQL queries for the campaign dashboard. They run against the committed simulated CSVs:

- `data/campaign_performance_sample.csv`
- `data/landing_page_sample.csv`
- `data/ab_test_conversion_sample.csv`

Run the full companion file from the repository root with:
...
pivoted AS (
    SELECT MAX(CASE WHEN variant = 'A' THEN conversion_rate END) AS control_rate,
           MAX(CASE WHEN variant = 'B' THEN conversion_rate END) AS treatment_rate
    FROM variants
)
SELECT treatment_rate - control_rate AS absolute_uplift,
